...
int pass_one(FILE* input, FILE* output, SymbolTable* symtbl) {
	...
	int flag=add_if_label(input_line,token,byte_offset,symtbl);
	if(flag==-1){
		ret_code=-1;
		continue;
	}else if(flag=1){
...
int pass_two(FILE *input, FILE* output, SymbolTable* symtbl, SymbolTable* reltbl) {
    ...
    while (fgets(buf, BUF_SIZE, input)) {
        ...
        char* args[MAX_ARGS];
        int num_args = 0;
	int flag=parse_args(input_line,args,&num_args);
	if(flag==-1)ret_code=-1;
        ...
        /* Use translate_inst() to translate the instruction and write to output file.
           ...
           raise_inst_error(). */
	int flag2=translate_inst(output,name,args,num_args,byte_offset,symtbl,reltbl);
	if(flag2==-1){
