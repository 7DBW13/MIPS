...
inst_needs_relocation:
	# two j type inst need relocation
	li $t0, 0x2 #test opcode for j
	li $t1, 0x3 #test opcode for jal
	srl $a0,$a0, 26
...
relocate_inst:
	...
	#somewhere is wrong... so I change
	#addiu $a0,$a3,0#load reloc tbl into a0
	#jal symbol_for_addr 
	#beq $v0,$0 Error#addr not in tabl, return -1
	
	#search symb tabl for name and its relating addr
	move $a1, $v0
	move $a0,$a2
	jal addr_for_symbol
	blt $v0, $0, error
